...
Sticker replies are DM-only, same as the AI text replies. When enabled, the
bot sends one randomly chosen sticker from the logged-in account's own
favorited + recently used stickers:

- After every AI text reply in a DM (a little extra personality).
- On its own, if the other person sends a sticker with no text.

The pool is loaded once at startup. If you favorite or use new stickers on
your account afterward, send `/refreshstickers` to reload the pool without
restarting the service. If your account has no favorited or recently used
stickers, sticker replies are silently skipped.
