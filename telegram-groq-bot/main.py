...
SYSTEM_PROMPT = os.getenv(
    "GROQ_SYSTEM_PROMPT",
    (
        "You are a warm, friendly Telegram direct-message assistant chatting on "
        "behalf of Rohan while he's away. Chat naturally and make the conversation "
        "feel relaxed, kind, and personal. Show genuine interest by asking "
        "thoughtful follow-up questions when appropriate. Keep replies "
        "conversational and not overly long. Use light humor when it fits, but "
        "never be rude, pushy, or judgmental. "
        "If the user asks where Rohan is, whether he's around, or anything about "
        "his availability, reply naturally and briefly that he's offline right "
        "now and will get back to them when he's back online. Don't say this "
        "unless they actually ask about him. "
...
# Cache of the account's own sticker documents, refreshed at startup so a
# random pick doesn't need a network round trip on every DM.
sticker_pool: list[Any] = []


async def refresh_sticker_pool() -> None:
    """Load the logged-in account's favorited + recently used stickers."""
    seen_ids: set[int] = set()
    stickers: list[Any] = []

    try:
        faved = await client(GetFavedStickersRequest(hash=0))
        for doc in getattr(faved, "stickers", []) or []:
            if doc.id not in seen_ids:
                seen_ids.add(doc.id)
                stickers.append(doc)
    except Exception:
        logger.exception("Failed to load favorited stickers")

    try:
        recent = await client(GetRecentStickersRequest(hash=0))
        for doc in getattr(recent, "stickers", []) or []:
            if doc.id not in seen_ids:
                seen_ids.add(doc.id)
                stickers.append(doc)
    except Exception:
        logger.exception("Failed to load recent stickers")

    sticker_pool[:] = stickers
    logger.info("Loaded %s stickers from the account's own sticker pack", len(sticker_pool))
...
async def handle_control_command(
    ...
    global auto_reply_enabled, sticker_reply_enabled, current_system_prompt
    ...
    if command == "/status":
        ...
        sticker_state = "ON" if sticker_reply_enabled else "OFF"
        await event.respond(
            f"Auto-replies: {state}\n"
            f"Sticker replies: {sticker_state} ({len(sticker_pool)} cached)\n"
            f"Model: {GROQ_MODEL}\n"
            f"Owner ID: {owner_user_id}"
        )
        return True

    if command in {"/stickerson"}:
        sticker_reply_enabled = True
        await event.respond("Sticker replies are ON.")
        return True

    if command in {"/stickersoff"}:
        sticker_reply_enabled = False
        await event.respond("Sticker replies are OFF.")
        return True

    if command == "/refreshstickers":
        await refresh_sticker_pool()
        await event.respond(f"Reloaded {len(sticker_pool)} stickers from your pack.")
        return True
    ...
    if command == "/help":
        await event.respond(
            ...
            "/stickerson - enable random sticker replies in DMs\n"
            "/stickersoff - disable random sticker replies\n"
            "/refreshstickers - reload your favorited/recent stickers"
        )
        return True
...
async def send_random_sticker(event: events.NewMessage.Event, user_id: int) -> None:
    """Send one randomly chosen sticker from the account's own sticker pack."""
    if not sticker_pool:
        return
    sticker = random.choice(sticker_pool)
    try:
        await event.respond(file=sticker)
        logger.info("Sent random sticker reply to user %s", user_id)
    except Exception:
        logger.exception("Failed to send sticker reply to user %s", user_id)
...
async def handle_message(event: events.NewMessage.Event) -> None:
    ...
    # A sticker DM has no text to send to Groq, so just answer it with a
    # random sticker from the account's own pack instead of falling through
    # to the empty-text check below. Still DM-only and still respects both
    # the auto-reply and sticker-reply toggles.
    if event.sticker and not text:
        if not (auto_reply_enabled and sticker_reply_enabled):
            return
        await send_random_sticker(event, user_id)
        return
